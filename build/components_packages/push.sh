...
for component in "${components[@]}"; do
  package_file_name="${component}_${package_tag}.tgz"
...
log "start pushing the components packages"
...
log "loop on the components for which we want to upload the package"
for component in "${components[@]}"; do
  log "upload component package for '$component'"
  package_file_name="${component}_${package_tag}.tgz"
  rclone copy \
   "$here/packages/$package_file_name" \
   "$rclone_remote_name:$infra_bucket/components_packages/$package_tag"
  log "the package is uploaded"
done

log "all components packages are uploaded"
