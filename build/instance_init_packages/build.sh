...
for instance in "${instances[@]}"; do
  assert_file_existence "$here/$instance/build.sh"
done
...
log "start preparing the init instance packages"
...
log "loop on the instances"
for instance in "${instances[@]}"; do
  log "create the instance init package for '$instance'"
  "$here/$instance/build.sh" "$package_tag"
  log "the package is ready"
done

log "the init packages for all the instances are ready"
