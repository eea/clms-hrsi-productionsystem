...
docker_image_for_rlies1_processing=$( echo "$system_parameters" | jq -r '.docker_image_for_rliepart2_processing' )

echo "make sure the docker image for test_job processing is available"
"$here/ensure_docker_image.sh" "$docker_image_for_rlies1_processing"


python3 -c "
import components.worker.python.run_rlies1_worker as run_rlies1_worker
run_rlies1_worker.main($job_id)
"