...
docker_image_for_test_job_processing=$( echo "$system_parameters" | jq -r '.docker_image_for_test_job_processing' )

echo "make sure the docker image for test_job processing is available"
"$here/ensure_docker_image.sh" "$docker_image_for_test_job_processing"


python3 -c "
import components.worker.python.run_test_job_worker as run_test_job_worker
run_test_job_worker.main($job_id)
"