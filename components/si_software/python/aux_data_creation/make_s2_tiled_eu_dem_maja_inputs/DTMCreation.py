...
from aux_data_creation.make_s2_tiled_eu_dem_maja_inputs import conversion_format_maja as conversion
...
class DTMCreator():
    ...
    def __init__(self, dirProduct, kml, granuleID, dirSRTM, dirWater, waterOnly=False, mntType="SRTM", coarseRes=240, zone_selection_shapefile=None):
        """
        ...
        if(files):
            ...
            if dirWater is not None:
                if os.path.isdir(dirWater):
        ...
        self.zone_selection_shapefile = zone_selection_shapefile
        self.example_l1c_file = None
        ...
        if(dirProduct):
            ...
            if self.mntType == 'EUDEM':
                dir_l1c_data = '%s/IMG_DATA'%os.path.dirname(self.mtd)
                example_l1c_files = ['%s/%s'%(dir_l1c_data, el) for el in os.listdir(dir_l1c_data) if 'B01.jp2' in el]
                if len(example_l1c_files) == 0:
                    raise Exception('Could not retrieve example L1C file necessary for EUDEM processing')
                self.example_l1c_file = example_l1c_files[0]
            self.mode = self.modeMTD
        elif self.mntType == 'EUDEM':
            raise Exception('EUDEM tile can only be computed from L1C product, not from kml file')
    ...
    def getSiteInfo(self, mtd, dtype):
        ...
        from aux_data_creation.make_s2_tiled_eu_dem_maja_inputs.tuilage_mnt_eau_S2 import TuilageParamsConverter
        if(dtype == self.typeNatif):
            ...
            assert tileFilenameItems[-2][0] == "T"
            tileName = str(tileFilenameItems[-2][1:])
            #EPSG_out e.g. 32632
    ...
    def run(self, outdir, tempout):
        ...
        from aux_data_creation.make_s2_tiled_eu_dem_maja_inputs import tuilage_mnt_eau_S2 as tiling
        mntcreator = tiling.TuilageSentinel()
        output_status = mntcreator.run(self.dirInSRTM, self.dirInWater, tempout, tempout, self.coarseRes, site=self.site, mnt=self.mntType, waterOnly=self.waterOnly, wdir=tempout,
                       water_zipped = self.WaterZipped, example_l1c_file=self.example_l1c_file)
        
        if output_status != 'valid':
            return output_status
        
        if not self.waterOnly:
            converter = conversion.MAJAConverter()
            converter.run(self.site.nom, tempout, self.coarseRes, outdir, zone_selection_shapefile=self.zone_selection_shapefile)
            
        replicate_directory_with_compressed_tiffs(outdir, tiled=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-w", "--water",help="The path to the folder containing the extracted Water-Body shapefiles. If it is a shapefile instead of a folder, " + \
        "this single shapefile will be used. If this argument is not filled, the program will assume that the tile does not contain land water.", required=False, type=str)
    parser.add_argument("--wateronly",help="Process water only. Default is False", required=False, action='store_true', default=False)
    parser.add_argument("-m", "--mnt",help="MNT type : SRTM (default), EUDEM or PO (Planet Observer).", required=False, type=str, default='SRTM')
    parser.add_argument("-c", "--coarseres",help="Coarse resolution. Default is 240", required=False, type=int, default=240)
    ...
    parser.add_argument("--zone_selection", help="Zone selection shapefile", required=False, type=str)
    args = parser.parse_args()

    creator = DTMCreator(args.input, args.kml, args.granule, args.srtm, args.water, waterOnly=args.wateronly, mntType=args.mnt, coarseRes=args.coarseres, \
        zone_selection_shapefile=args.zone_selection)
